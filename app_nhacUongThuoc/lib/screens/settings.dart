...
class _SettingsState extends State<Settings> {
  ...
  Future<void> _loadUserInfo() async {
    try {
      ...
      print('User info from API: $userInfo');

      if (mounted && userInfo != null) {
        setState(() {
          // Ưu tiên lấy từ userInfo
          _username = userInfo['username'] ?? 'Người dùng';
  ...
  Future<void> _navigateToAvatarSelection() async {
    ...
    if (result != null) {
      // Reload user info sau khi đổi avatar
      await _loadUserInfo();
    }
  }
  ...
  Future<void> _handleLogout() async {
    ...
    if (confirmed == true) {
      await ApiService.instance.clearUserId();
      if (mounted) {
